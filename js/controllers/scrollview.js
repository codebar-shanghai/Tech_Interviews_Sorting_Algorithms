...
export default class ScrollView {
            ...
            if (
                ...
                if (isVertical && horizontalBackgrounds.length > h) {
                    ...
                    const pageBackground =
                        window.getComputedStyle(slideBackground);
    ...
    syncPages() {
        ...
        this.viewportElement.style.scrollSnapType =
            typeof config.scrollSnap === 'string'
                ? `y ${config.scrollSnap}`
                : '';
        ...
        this.pages = pageElements.map((pageElement) => {
            const page = this.createPage({
                ...
                backgroundElement:
                    pageElement.querySelector('.slide-background'),
            ...
            let totalScrollTriggerCount = Math.max(
                page.scrollTriggers.length - 1,
                0
            );

            // Each auto-animate step may include its own scroll triggers
            // for fragments, ensure we count those as well
            totalScrollTriggerCount += page.autoAnimatePages.reduce(
                (total, page) => {
    ...
    createAutoAnimateTriggersForPage(page) {
        if (page.autoAnimateElements.length > 0) {
            ...
            this.slideTriggers.push(
                ...Array.from(page.autoAnimateElements).map(
                    (autoAnimateElement, i) => {
                        let autoAnimatePage = this.createPage({
                            slideElement:
                                autoAnimateElement.querySelector('section'),
                            contentElement: autoAnimateElement,
                            backgroundElement:
                                autoAnimateElement.querySelector(
                                    '.slide-background'
                                ),
                        });
                        ...
                        return {
                            ...
                            deactivate: () =>
                                this.deactivatePage(autoAnimatePage),
                        };
                    }
                )
            );
    ...
    syncProgressBar() {
        ...
        if (progressSegmentHeight > MIN_PROGRESS_SEGMENT_HEIGHT) {
            this.slideTriggers.forEach((slideTrigger) => {
                ...
                page.scrollTriggerElements = page.scrollTriggers.map(
                    (trigger, i) => {
    ...
    scrollToSlide(slideElement) {
        ...
        if (!this.active) {
            this.activatedCallbacks.push(() =>
                this.scrollToSlide(slideElement)
            );
        } else {
    ...
    activatePage(page) {
        if (!page.active) {
            ...
            Array.from(
                contentElement.parentNode.querySelectorAll(
                    '.scroll-page-content'
                )
            ).forEach((sibling) => {
